...
/***************************************************************************//**
 *  Add text data to the packet.
 ******************************************************************************/
void bthome_v2_add_text(const char text[]) {
  if (!sl_str_is_empty(text)) {
    ...
    if ((sensor_data_index + 2 + text_length)
        <= (MEASUREMENT_MAX_LEN - (b_encrypt_enable ? 8 : 0))) {
      sensor_data[sensor_data_index] = ID_TEXT & 0xff;
      sensor_data_index++;
      sensor_data[sensor_data_index] = text_length & 0xff;
      sensor_data_index++;
      // Convert each character to hexadecimal and store in sensor_data
      for (size_t i = 0; i < text_length; i++) {
        uint8_t ascii_value = (uint8_t)text[i];
        sensor_data[sensor_data_index] = ascii_value;
        sensor_data_index++;
      }
      if (!b_sort_enable) {
        if (ID_TEXT < last_object_id) {
          b_sort_enable = true;
        }
      }
      last_object_id = ID_TEXT;
    } else {
      bthome_v2_send_packet();
